import { SatelliteData, SatelliteObservation } from '../../types';

const STAC_BASE_URL = process.env.COPERNICUS_STAC_URL || 'https://stac.dataspace.copernicus.eu/v1';
const STAC_SEARCH_URL = `${STAC_BASE_URL.replace(/\/$/, '')}/search`;
const STAC_BROWSER_URL = 'https://browser.stac.dataspace.copernicus.eu';
...
function getProperty<T>(item: StacItem, key: string): T | undefined {
  return item.properties?.[key] as T | undefined;
}

function findItemUrl(item: StacItem): string | undefined {
  const self = item.links?.find(link => link.rel === 'self')?.href;
  return self || (item.id ? `${STAC_BROWSER_URL}/collection/${item.collection}/item/${encodeURIComponent(item.id)}` : undefined);
}
...
function observationPoint(item: StacItem): [number, number] | undefined {
  ...
  if (Array.isArray(bbox) && bbox.length >= 4) {
    return [Number((bbox[1] + bbox[3]) / 2), Number((bbox[0] + bbox[2]) / 2)];
  }
...
async function searchCollection(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Copernicus STAC ${response.status}: ${body.slice(0, 180)}`);
  }
...
async function searchFirstAvailable(
...
function toObservation(item: StacItem, lat: number, lon: number): SatelliteObservation {
  ...
  const distance = point ? distanceKm(lat, lon, point[0], point[1]) : undefined;
...
export async function fetchSatelliteData(
  ...
  // Satellite observations cannot provide a forecast. For a future marine window,
  // search for the most recent usable observation available before that window.
  const searchEnd = new Date(Math.min(requestedEnd.getTime(), now.getTime()));
  if (requestedStart.getTime() > requestedEnd.getTime()) {
    return buildNoObservation(lat, lon, ['Invalid satellite observation window: start time is after end time.']);
  }
  const searchStart = new Date(searchEnd.getTime() - 7 * 24 * 3600000);

  if (searchEnd <= searchStart) {
    return buildNoObservation(lat, lon, ['No valid satellite observation window could be constructed.']);
  }
  ...
  for (const item of [olci, wst, s1, s2]) {
    if (item) observations.push(toObservation(item, lat, lon));
  }

  if (!observations.length) {
    return buildNoObservation(lat, lon, [
  ...
  const acquisitionTime = primary.acquisitionTime;
  const observationAgeHours = ageHours(acquisitionTime);
  ...
  if (typeof cloudCoverPct === 'number' && cloudCoverPct > 60) {
    warnings.push(`Best matching optical observation has high cloud cover (${cloudCoverPct.toFixed(0)}%).`);
  }
