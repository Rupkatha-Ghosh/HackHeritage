...
import { AgentStepTrace, LanguageCode, OrcaAnalysisResponse, SatelliteData, RiskPrediction, WeatherData, OceanData, LocationInfo, TimeWindow, GisLayerData, EvidenceItem } from '../../src/types.ts';
...
function unavailableSatellite(location: LocationInfo): SatelliteData {
  return {
    status: 'UNAVAILABLE', satelliteName: 'No satellite source', processingTime: new Date().toISOString(), latitude: location.latitude, longitude: location.longitude,
...
export async function runOrcaAgentWorkflow(query: string, locationOverride?: string, timeOverride?: string, language: LanguageCode = 'en'): Promise<OrcaAnalysisResponse> {
  ...
  const startTrace = (agentName: AgentStepTrace['agentName'], inputSummary: string, taskId?: string, dependencies?: string[]) => {
    const trace: AgentStepTrace = { agentName, status: 'running', startedAt: new Date().toISOString(), inputSummary, outputSummary: '', logs: [`Started ${agentName} processing`], taskId, dependencies };
    traces.push(trace); return trace;
  };
  const finishTrace = (trace: AgentStepTrace, output: string, error?: string) => {
    trace.status = error ? 'failed' : 'completed'; trace.completedAt = new Date().toISOString(); trace.durationMs = Math.max(1, Date.now() - new Date(trace.startedAt).getTime()); trace.outputSummary = output; if (error) trace.error = error;
  };
  ...
  const result = await executeOrcaPlan(plan, {
    ...
    satellite: async () => {
      ...
      const window = resolveSatelliteObservationWindow(timeWindow);
      satellite = await fetchSatelliteData(location.latitude, location.longitude, window.startTime, window.endTime);
    ...
    risk: async () => {
      ...
      const mlRisk = await predictMarineRiskWithMl(realtime.weather, realtime.ocean, satellite, location);
      const rawRisk = mlRisk || calculateMarineRisk(realtime.weather, realtime.ocean, satellite, location);
      ...
      } else trace.logs.push('ML API unavailable; deterministic fallback used.');
    ...
    gis: async () => {
      ...
      const trace = startTrace('GisAgent', 'Generate GeoJSON hazard and navigation layers', 'gis', ['resolve_location_time', 'risk']);
      gisLayers = generateGisLayers(location, risk, realtime.ocean);
      finishTrace(trace, `${gisLayers.features.length} GeoJSON features generated.`);
    },
    evidence: async () => {
    ...
    onTaskFailure: (task, error) => {
      const trace = traces.find(t => t.taskId === task.id && t.status === 'running');
      if (trace) finishTrace(trace, `${task.label} failed`, error.message);
    }
  });

  // Emit explicit skipped traces for branches the planner intentionally omitted.
  for (const task of result.plan.tasks.filter(t => !t.enabled)) {
    const existing = traces.find(t => t.taskId === task.id);
    if (!existing && task.id !== 'synthesis') {
      const agentName: AgentStepTrace['agentName'] = task.id === 'satellite' ? 'SatelliteAgent' : task.id === 'gis' ? 'GisAgent' : 'EvidenceRetrieval';
      traces.push({ agentName, status: 'skipped', startedAt: new Date().toISOString(), completedAt: new Date().toISOString(), durationMs: 0, inputSummary: task.label, outputSummary: task.reason, logs: [`Skipped by planner: ${task.reason}`], taskId: task.id, dependencies: task.dependsOn });
    }
  }

  if (!location || !timeWindow || !realtime || !risk) {
    const failure = result.failures.map(f => `${f.taskId}: ${f.reason}`).join('; ');
    throw new Error(`ORCA-X agent execution could not complete required tasks.${failure ? ` ${failure}` : ''}`);
  }
  const satelliteDegraded = satellite.status !== 'LIVE';
  const ragDegraded = result.plan.tasks.find(t => t.id === 'evidence')?.status !== 'completed';
  ...
  return {
    queryId, originalQuery: query, language, detectedIntent: result.plan.intent, location, timeWindow,
    weather: realtime.weather, ocean: realtime.ocean, satellite, risk, gisLayers, evidence, agentTraces: traces, groundedSummary,
    executionPlan: { planId: result.plan.planId, intent: result.plan.intent, rationale: result.plan.rationale, tasks: result.plan.tasks, generatedAt: result.plan.generatedAt },
    isDataDegraded: realtime.degraded || satelliteDegraded || ragDegraded,
