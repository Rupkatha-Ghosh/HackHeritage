...
export async function runOrcaAgentWorkflow(query: string, locationOverride?: string, timeOverride?: string, language: LanguageCode = 'en'): Promise<OrcaAnalysisResponse> {
  ...
  const result = await executeOrcaPlan(plan, {
    ...
    synthesis: async () => {
      ...
      const trace = startTrace('ResponseGrounding', 'Generate grounded marine intelligence briefing', 'synthesis', plan.tasks.find(t => t.id === 'synthesis')?.dependsOn); const genAI = getGenAI();
      if (genAI) {
  ...
  const satelliteDegraded = satellite.status !== 'LIVE'; const evidenceTask = result.plan.tasks.find(t => t.id === 'evidence'); const ragDegraded = Boolean(evidenceTask && evidenceTask.status !== 'completed');
