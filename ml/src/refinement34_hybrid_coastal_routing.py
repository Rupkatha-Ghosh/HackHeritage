...
def predict_one(model: xgb.XGBRegressor, X_cpu: np.ndarray) -> np.ndarray:
    ...
    # XGBRegressor does not expose inplace_predict; its Booster does. Use the
    # sklearn predict API here because it is version-stable and correctly
    # handles the booster device configuration. This also avoids the previous
    # R34 AttributeError on XGBoost 3.4.1.
...
def main():
    ...
    route_table=pd.DataFrame([{"location":loc,"scenario":scenario,"selected_strategy":strategy} for (loc,scenario),strategy in sorted(router["routes"].items())]); route_table.to_csv(OUTPUT_DIR/"hybrid_route_table.csv",index=False)
