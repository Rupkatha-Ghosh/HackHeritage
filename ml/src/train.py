...
def load_dataset() -> pd.DataFrame:
    ...
    return df.dropna(subset=[TARGET_COLUMN]).sort_values("timestamp").copy()
...
def evaluate(model: xgb.XGBClassifier, X: pd.DataFrame, y: pd.Series, name: str) -> dict:
    ...
    report = classification_report(
        ...
        target_names=[RISK_CLASS_NAMES[i] for i in range(4)],
...
def main() -> None:
    ...
    # Spatial holdout: Digha is never used to fit this evaluation model.
    spatial_train = df[df["location_id"] != HOLDOUT_LOCATION].copy()
    spatial_test = df[df["location_id"] == HOLDOUT_LOCATION].copy()
    if spatial_test.empty:
        raise ValueError(f"Spatial holdout location {HOLDOUT_LOCATION!r} is missing.")

    # Temporal split inside the non-holdout locations.
    spatial_train = spatial_train.sort_values("timestamp")
    n = len(spatial_train)
    train_end = int(n * 0.70)
    validation_end = int(n * 0.85)
    train_df = spatial_train.iloc[:train_end]
    validation_df = spatial_train.iloc[train_end:validation_end]
    ...
    model.fit(
        ...
        eval_set=[(validation_df[FEATURE_COLUMNS], validation_df[TARGET_COLUMN].astype(int))],
        verbose=100,
    )
    ...
    # Final production fit uses all historical observations after unbiased evaluation.
    ...
    metadata = {
        ...
        "classes": {str(k): v for k, v in RISK_CLASS_NAMES.items()},
        "features": FEATURE_COLUMNS,
        "feature_count": len(FEATURE_COLUMNS),
        "random_state": RANDOM_STATE,
        "evaluation": {
            "temporal_split": "70/15 on non-Digha locations ordered by timestamp",
            "spatial_holdout": HOLDOUT_LOCATION,
            "temporal_validation": validation_metrics,
            "spatial_holdout": spatial_metrics,
        },
        "training_rows": int(len(df)),
        "training_locations": sorted(df["location_id"].unique().tolist()),
        "class_weights": {str(k): float(v) for k, v in final_weights.items()},
        "hyperparameters": {
            "n_estimators": 700, "learning_rate": 0.05, "max_depth": 8,
            "min_child_weight": 3, "subsample": 0.85,
            "colsample_bytree": 0.85, "reg_alpha": 0.05,
            "reg_lambda": 1.0, "gamma": 0.0,
        },
        "feature_importance": {feature: float(score) for feature, score in importance},
        "label_policy": "ORCA-X operational proxy anchored to IMD/RSMC 25/34 kt marine wind bands and WMO Douglas sea-state terminology.",
        "deployment_validation": "INDIAN_COASTAL_HISTORICAL_PROXY_VALIDATED_NOT_A_STATUTORY_WARNING",
        "warning": "The target is an operational severity proxy, not historical vessel incidents or official IMD/INCOIS warning labels. RAG/authoritative advisories remain the final safety evidence layer.",
    }
